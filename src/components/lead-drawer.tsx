...
export function LeadDrawer({ lead, employeeName, onClose, onUpdate, notify }: { lead:Lead; employeeName:string; onClose:()=>void; onUpdate:(lead:Lead)=>void; notify:(message:string)=>void }) {
  ...
  function saveStatus(status: LeadStatus, latest: string, nextAction: string, addAttempt = true) {
    const nextAttempts = lead.attempts + (addAttempt ? 1 : 0);
    const nextDays = Math.min(3, lead.attemptDays + (addAttempt ? 1 : 0));
    const reachesThree = ["No Answer","WhatsApp Sent","Call Back Later"].includes(status) && nextDays >= 3;
    const updated: Lead = {
      ...lead,
      status: reachesThree ? "Manager Review" : status,
    ...
    setOutcome("");
    setNotes("");
    notify(reachesThree ? "Three unsuccessful days reached - moved to Manager Review" : `${lead.patient}: ${latest}`);
  }
  ...
  function recordCall() {
    ...
    if (outcome === "No answer") saveStatus("No Answer", "No answer", "Tomorrow");
    else if (outcome === "Interested - needs availability") saveStatus("Waiting for Patient Response", "Patient requested availability", "Follow up in 2 days");
    else saveStatus("Call Attempted", outcome, "Next follow-up due");
  }
  ...
  return <><div className="drawer-backdrop" onClick={onClose}/><aside className="drawer">
    ...
    <div className="drawer-body">
      ...
      <section className="drawer-section"><div className="section-head"><strong>Choose the next patient action</strong><span style={{fontSize:8,color:"#8a9b98"}}>Every action is audited</span></div><div className="section-body"><div className="action-grid">
        ...
        <button className="action-btn warn" onClick={()=>saveStatus("No Answer","No answer","Tomorrow")}><Phone size={18}/>Mark No Answer</button>
  ...
  {workflow === "callback" && <WorkflowModal title="Schedule patient callback" onClose={()=>setWorkflow(null)} action={<button className="btn btn-primary" onClick={()=>saveStatus("Call Back Later","Callback scheduled",`${callbackDate} - 10:00`)}><CalendarClock size={13}/>Schedule callback</button>}><div className="form-grid"><div className="form-field"><label>Callback date *</label><input required className="form-control" type="date" value={callbackDate} onChange={e=>setCallbackDate(e.target.value)}/></div><div className="form-field"><label>Time or time range *</label><select className="form-control"><option>10:00-12:00</option><option>08:00-10:00</option><option>12:00-14:00</option><option>14:00-16:00</option></select></div><div className="form-field full"><label>Reason *</label><select className="form-control"><option>Patient is currently busy</option><option>Patient requested a different day</option><option>Needs to check calendar</option><option>Discuss with family / medical aid</option></select></div><div className="form-field full"><label>Notes</label><textarea className="form-control" placeholder="Helpful context for the callback..."/></div></div><div className="callout" style={{marginTop:12,marginBottom:0}}><Info size={14}/><span>This lead will automatically return to the employee's due list on the callback date.</span></div></WorkflowModal>}
  {workflow === "booking" && <WorkflowModal title="Record patient booking request" onClose={()=>setWorkflow(null)} action={<button className="btn btn-primary" disabled={!bookingDate} onClick={()=>saveStatus("Booking Recorded Pending Verification",`Booking recorded: ${bookingDate}`,"Awaiting manager verification")}><Check size={13}/>Record booking</button>}><div className="callout"><Info size={14}/><span>This records the patient's preferred appointment. A manager must find it on the official practice calendar before it becomes verified.</span></div><div className="form-grid"><div className="form-field"><label>Preferred booking date *</label><input required className="form-control" type="date" value={bookingDate} onChange={e=>setBookingDate(e.target.value)}/></div><div className="form-field"><label>Preferred time / range *</label><select className="form-control"><option>10:00-12:00</option><option>Morning</option><option>Afternoon</option><option>Any available time</option></select></div><div className="form-field full"><label>Booking confidence *</label><select className="form-control"><option>Confirmed with patient</option><option>Patient requested availability</option><option>Tentative</option></select></div><div className="form-field full"><label>Booking notes</label><textarea className="form-control" placeholder="Patient preferences, practitioner, appointment type..."/></div></div></WorkflowModal>}
  {(workflow === "not-interested" || workflow === "wrong-number" || workflow === "escalate") && <WorkflowModal title={workflow === "not-interested"?"Confirm patient is not interested":workflow === "wrong-number"?"Confirm wrong number":"Escalate to manager"} onClose={()=>setWorkflow(null)} action={<button className={`btn ${workflow === "escalate"?"btn-primary":"btn-danger-soft"}`} onClick={()=>saveStatus(workflow === "escalate"?"Manager Review":workflow === "wrong-number"?"Wrong Number Confirmed":"Patient Not Interested",workflow === "escalate"?"Escalated to manager":workflow === "wrong-number"?"Wrong number confirmed":"Patient not interested",workflow === "escalate"?"Manager review":"Completed")}><Check size={13}/>Confirm outcome</button>}><div className="callout" style={{background:workflow === "escalate"?"#edf6ff":"#fff4ed"}}><AlertTriangle size={14}/><span>{workflow === "escalate"?"The lead will remain active while a manager reviews it.":"This is a final outcome. Add enough detail for a manager to audit the decision."}</span></div><div className="form-field"><label>{workflow === "escalate"?"Escalation reason *":"Reason *"}</label><select className="form-control"><option>{workflow === "not-interested"?"Patient clearly declined recall":workflow === "wrong-number"?"Number belongs to another person":"Patient requested manager contact"}</option><option>Clinical or sensitive concern</option><option>Repeated contact difficulty</option><option>Data quality issue</option></select></div><div className="form-field" style={{marginTop:11}}><label>Required notes</label><textarea className="form-control" value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Record what was confirmed and how..."/></div></WorkflowModal>}
  </>;
}
